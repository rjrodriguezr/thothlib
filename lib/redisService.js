...
class RedisService {
    ...
    async close() {
        ...
        if (this.isClosing) {
            logger.trace('Cierre de Redis ya en progreso, evitando múltiples llamadas.');
        ...
        if (this.client && this.client.status !== 'end' && this.client.status !== 'disconnecting') {
            logger.trace('Cerrando el cliente Redis...');
            try {
    ...
    async setData(key, value, options = []) {
        ...
        try {
            ...
            logger.trace(`Datos guardados en Redis para la clave "${key}".`);
            return result; // 'OK'
        } catch (error) {
    ...
    async getData(key) {
        ...
        try {
            ...
            if (value === null) {
                logger.trace(`No se encontró valor en Redis para la clave "${key}".`);
            ...
            try {
                ...
                logger.trace(`Datos recuperados y parseados (JSON) de Redis para la clave "${key}".`);
                return parsedValue;
            } catch (e) {
                // Si falla el parseo JSON, es probable que sea un string simple, número o booleano guardado como string.
                logger.trace(`Datos recuperados (string) de Redis para la clave "${key}". No se pudo parsear como JSON, devolviendo valor crudo.`);
                return value;
            }
        } catch (error) {
    ...
    async delData(keys) {
        ...
        try {
            ...
            logger.trace(`Clave(s) eliminada(s) de Redis: ${Array.isArray(keys) ? keys.join(', ') : keys}. Resultado: ${result}`);
    ...
    async setupStreamGroup(streamKey, groupName) {
        ...
        try {
            ...
            if (error.message.includes('BUSYGROUP')) {
                logger.trace(`El grupo '${groupName}' ya existe en el stream '${streamKey}'. No se requiere acción.`);
     ...
     * @param {object} [options={}] - Opciones para XADD. Ej: { strategy: 'MINID', threshold: 1672531200000 }
     * @param {string} [options.strategy] - Estrategia de trimming ('MINID' o 'MAXLEN').
     * @param {number|string} [options.threshold] - El umbral para la estrategia de trimming.
     * @param {boolean} [options.approximate] - Para MAXLEN, si el trimming es aproximado (usando '~').
     * @returns {Promise<string|null>} El ID del mensaje creado, o null si hay un error.
     */
    async publishToStream(streamKey, messagePayload, options = {}) {
        if (!this.client || this.client.status !== 'ready') {
        ...
        try {
            ...
            const xaddArgs = [];
            if (options.strategy && options.threshold) {
                xaddArgs.push(options.strategy);
                if (options.approximate) {
                    xaddArgs.push('~');
                }
                xaddArgs.push(options.threshold);
            }
            xaddArgs.push('*', 'payload', payloadString);

            const messageId = await this.client.xadd(streamKey, ...xaddArgs);
            logger.debug(`Mensaje publicado en stream '${streamKey}' con ID: ${messageId}`);
    ...
    async readFromStreamGroup(streamKey, groupName, consumerName, options = {}) {
        ...
        try {
            ...
            logger.debug(`Leyendo stream '${streamKey}:${streamName}' para grupo '${groupName}' (consumidor: ${consumerName}). Origen: ${readFromId === '0' ? 'pendientes' : 'nuevos'}.`);
            logger.debug(JSON.stringify(stream));
    ...
    async ackStreamMessage(streamKey, groupName, messageIds) {
        ...
        try {
            ...
            logger.trace(`ACK enviado para ${Array.isArray(messageIds) ? messageIds.length : 1} mensaje(s) en '${streamKey}'.`);
            return result;
        } catch (error) {
    ...
    async publish(channel, payload) {
        ...
        try {
            ...
            logger.trace(`Mensaje publicado en canal '${channel}'. Recibido por ${receivers} subscriptores.`);
            return receivers;
        } catch (error) {
