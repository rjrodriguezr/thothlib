...
class RedisService {
    ...
    async publishToStream(streamKey, messagePayload) {
        ...
        try {
            ...
            logger.silly(`Mensaje publicado en stream '${streamKey}' con ID: ${messageId}`);
    ...
    async readFromStreamGroup(streamKey, groupName, consumerName, options = {}) {
        ...
        try {
            ...
            //logger.silly(`Leyendo stream '${streamKey}' para grupo '${groupName}' (consumidor: ${consumerName}). Origen: ${readFromId === '0' ? 'pendientes' : 'nuevos'}.`);
